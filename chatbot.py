...
def confirm_intent_save(text, response):
    ...
    SAFE VERSION (no infinite loop freeze issues)
    """

    print("Detected unmatched intent. Save this interaction? (yes/no)")
    speak_macos("I found a new pattern. Do you want to save it?")

    # try limited attempts only (IMPORTANT FIX)
    attempts = 0
    max_attempts = 3
    ...
    while attempts < max_attempts:
        ...
        if confirmation in ['yes', 'yeah', 'yep', 'sure', 'okay', 'ok']:
            ...
            if result:
                ...
                speak_macos("Saved successfully")
                return True
            else:
            ...
            print("Intent not saved.")
            speak_macos("Okay, not saved")
            return False

        else:
            print("Please say yes or no.")
            speak_macos("Please say yes or no.")

    # if user never responds properly
    print("No valid response received. Skipping save.")
    speak_macos("Skipping save")
    return False
...
def chat():
    ...
    while True:
        ...
        # ONLY use AI if intent NOT found
        if response is None:
            ...
            if not response or response in ["I didn't understand", "Error generating response"]:
                response = "Sorry, I didn't understand that."

        # Add response to conversation history (ONLY ONCE)
